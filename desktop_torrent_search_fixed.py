#!/usr/bin/env python3
"""
TorrentHunt Desktop - Version corrigée pour recherches multiples
Bug fix: Sessions HTTP correctement gérées entre recherches
"""
...
class TorrentHuntAPI:
    """Client pour l'API TorrentHunt avec gestion corrigée des sessions"""
    ...
    async def create_fresh_session(self):
        """Créer une nouvelle session HTTP fraîche pour chaque recherche"""
        headers = {
            'User-Agent': 'TorrentHunt Desktop Fixed/1.0',
            'Accept': 'application/json',
            'Connection': 'close'  # Force fresh connections
        }

        # Créer un connecteur avec des paramètres stricts
        ...
        print(f"[DEBUG] Nouvelle session HTTP créée")
        return session

    async def search_site(self, query: str, site: str) -> List[Dict]:
        """Rechercher sur un site spécifique avec session fraîche"""
        session = None
        try:
            print(f"[DEBUG] Recherche {site} pour: {query}")

            # Créer une session fraîche pour cette requête
            ...
            async with session.get(url, params=params) as response:
                print(f"[DEBUG] {site} - Status: {response.status}")

                if response.status == 200:
                    data = await response.json()
                    print(f"[DEBUG] {site} - Data type: {type(data)}")
                    ...
                    if "data" in data and data["data"]:
                        ...
                        print(f"[DEBUG] {site} - Trouvé {len(torrents)} torrents")
                        return torrents
                    else:
                        print(f"[DEBUG] {site} - Aucun résultat dans data: {data}")
                        return []
                else:
                    print(f"[DEBUG] {site} - Erreur HTTP {response.status}")
                    error_text = await response.text()
                    print(f"[DEBUG] {site} - Erreur: {error_text[:200]}...")
                    return []

        except Exception as e:
            print(f"[DEBUG] Erreur {site}: {e}")
            return []

        finally:
            # IMPORTANT: Toujours fermer la session
            if session:
                await session.close()
                print(f"[DEBUG] Session fermée pour {site}")

    async def search_selected_sites(self, query: str, selected_sites: List[str]) -> List[Dict]:
        """Rechercher sur les sites sélectionnés avec sessions indépendantes"""
        print(f"[DEBUG] === NOUVELLE RECHERCHE ===")
        print(f"[DEBUG] Query: {query}")
        print(f"[DEBUG] Sites: {selected_sites}")

        # Lancer les recherches en parallèle avec sessions indépendantes
        tasks = []
        ...
        try:
            print(f"[DEBUG] Lancement de {len(tasks)} tâches parallèles")
            ...
            for i, site_results in enumerate(results):
                ...
                if isinstance(site_results, list):
                    ...
                    print(f"[DEBUG] ✓ {site}: {len(site_results)} résultats")
                elif isinstance(site_results, Exception):
                    print(f"[DEBUG] ✗ {site}: {site_results}")

            print(f"[DEBUG] Total final: {len(all_torrents)} torrents")
            return all_torrents
...
class TorrentHuntGUI:
    """Interface graphique TorrentHunt Desktop avec bug fix"""

    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🏴‍☠️ TorrentHunt Desktop FIXED - Recherches multiples")
    ...
    def create_header(self, parent):
        ...
        title_label = ttk.Label(header_frame, text="🏴‍☠️ TorrentHunt Desktop FIXED", style='Title.TLabel')
        title_label.pack(side=tk.LEFT)

        subtitle_label = ttk.Label(header_frame, text="🐛 Bug recherches multiples corrigé • Tri • Sélection",
                                 font=('Arial', 10, 'italic'), foreground='#27ae60')
    ...
    def create_search_frame(self, parent):
        ...
        # Bouton de recherche
        search_btn = ttk.Button(search_frame, text="🚀 Rechercher (recherches multiples OK)",
    ...
    def create_status_bar(self, parent):
        ...
        self.status_var = tk.StringVar(value="🟢 Prêt - Recherches multiples corrigées !")
...
{torrent['magnet'][:100]}...
    ...
    def search_torrents(self):
        ...
        print(f"[GUI] Lancement recherche: {query} sur {len(selected_sites)} sites")
    ...
    def run_search(self, query: str, selected_sites: List[str]):
        ...
        print(f"[GUI] Thread de recherche démarré")
        ...
        # IMPORTANT: Créer une nouvelle boucle d'événements pour chaque recherche
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)

        try:
            start_time = time.time()
            print(f"[GUI] Début recherche async")
            results = loop.run_until_complete(self.api.search_selected_sites(query, selected_sites))
            end_time = time.time()

            search_time = end_time - start_time
            print(f"[GUI] Recherche terminée: {len(results)} résultats en {search_time:.1f}s")

            self.root.after(0, lambda: self.display_results(results, query, search_time, selected_sites))

        except Exception as e:
            print(f"[GUI] Erreur dans run_search: {e}")
            self.root.after(0, lambda: self.show_error(f"Erreur lors de la recherche: {str(e)}"))
        finally:
            # IMPORTANT: Fermer la boucle d'événements
            loop.close()
            print(f"[GUI] Loop fermée")
    ...
    def display_results(self, results: List[Dict], query: str, search_time: float, selected_sites: List[str]):
        ...
        print(f"[GUI] Affichage des résultats: {len(results)} torrents")
    ...
    def on_closing(self):
        ...
        print(f"[GUI] Fermeture de l'application")
        self.root.destroy()
...
def main():
    ...
    try:
        print("🏴‍☠️ Lancement de TorrentHunt Desktop FIXED...")
        print("🐛 Bug recherches multiples corrigé")
